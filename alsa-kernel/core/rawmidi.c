...
static struct snd_rawmidi *snd_rawmidi_search(struct snd_card *card, int device)
{
	...
	list_for_each_entry(rawmidi, &snd_rawmidi_devices, list, struct snd_rawmidi)
				     ...
				     struct snd_rawmidi_info *info)
{
	...
	if (info->subdevice >= pstr->substream_count)
		return -ENXIO;
	list_for_each_entry(substream, &pstr->substreams, list, struct snd_rawmidi_substream) {
				       ...
				       struct snd_info_buffer *buffer)
{
	...
	if (rmidi->info_flags & SNDRV_RAWMIDI_INFO_OUTPUT) {
		list_for_each_entry(substream,
				    ...
				    list, struct snd_rawmidi_substream) {
			snd_iprintf(buffer,
				    "Output %d\n"
				    "  Tx bytes     : %lu\n",
				    substream->number,
...
static int snd_rawmidi_dev_disconnect(struct snd_device *device)
{
	...
	for (dir = 0; dir < 2; dir++) {
		...
		list_for_each_entry(s, &rmidi->streams[dir].substreams, list, struct snd_rawmidi_substream) {
			if (s->runtime)
			 ...
			 const struct snd_rawmidi_ops *ops)
{
	...
	list_for_each_entry(substream, &rmidi->streams[stream].substreams, list, struct snd_rawmidi_substream)
		substream->ops = ops;
}
