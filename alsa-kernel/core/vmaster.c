...
static int master_init(struct link_master *master)
{
	...
	list_for_each_entry(follower, &master->followers, list, struct link_follower) {
			  ...
			  unsigned int flags)
{
	...
	srec = kzalloc(sizeof(*srec) +
		       follower->count * sizeof(*follower->vd), GFP_KERNEL);
#endif
	if (!srec)
		return -ENOMEM;
	srec->kctl = follower;
...
static void master_free(struct snd_kcontrol *kcontrol)
{
	...
	list_for_each_entry_safe(follower, n, &master->followers, list, struct link_follower) {
