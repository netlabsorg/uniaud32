...
#else
		status32.state = status64.state;
		status32.trigger_tstamp_sec = status64.trigger_tstamp_sec;
		status32.trigger_tstamp_nsec = status64.trigger_tstamp_nsec;
		status32.tstamp_sec = status64.tstamp_sec;
		status32.tstamp_nsec = status64.tstamp_nsec;
		status32.appl_ptr = status64.appl_ptr;
		status32.hw_ptr = status64.hw_ptr;
		status32.delay = status64.delay;
		status32.avail = status64.avail;
		status32.avail_max = status64.avail_max;
		status32.overrange = status64.overrange;
		status32.suspended_state = status64.suspended_state;
		status32.audio_tstamp_data = status64.audio_tstamp_data;
		status32.audio_tstamp_sec = status64.audio_tstamp_sec;
		status32.audio_tstamp_nsec = status64.audio_tstamp_nsec;
		status32.driver_tstamp_sec = status64.audio_tstamp_sec;
		status32.driver_tstamp_nsec = status64.audio_tstamp_nsec;
		status32.audio_tstamp_accuracy = status64.audio_tstamp_accuracy;
#endif
...
#else
	struct fd f;
//FIXME 	f.file = (struct file *)substream->file;
#endif
			 ...
			 unsigned int cmd, void *arg)
{
	...
	case SNDRV_PCM_IOCTL_DROP:
		return snd_pcm_drop(substream);
	case SNDRV_PCM_IOCTL_DELAY:
		return snd_pcm_delay(substream, (snd_pcm_sframes_t*)frames);
	default:
		return -EINVAL;
	}
}
