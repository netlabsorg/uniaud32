...
static void stac_store_hints(struct hda_codec *codec)
{
	...
	if (get_int_hint(codec, "gpio_mask", (int*) &spec->gpio_mask)) {
		spec->eapd_mask = spec->gpio_dir = spec->gpio_data =
			spec->gpio_mask;
	}
	if (get_int_hint(codec, "gpio_dir", (int*) &spec->gpio_dir))
		spec->gpio_dir &= spec->gpio_mask;
	if (get_int_hint(codec, "gpio_data", (int*) &spec->gpio_data))
		spec->gpio_data &= spec->gpio_mask;
	if (get_int_hint(codec, "eapd_mask", (int*) &spec->eapd_mask))
		spec->eapd_mask &= spec->gpio_mask;
	if (get_int_hint(codec, "gpio_mute", (int*) &spec->gpio_mute))
...
//	const struct dmi_device *dev = NULL;
	...
	if (get_int_hint(codec, "gpio_led", (int*) &spec->gpio_led)) {
		get_int_hint(codec, "gpio_led_polarity",
			     (int*) &spec->gpio_led_polarity);
		return 1;
	}
...
#ifdef TARGET_OS2
static const struct hda_verb STAC_9200_EAPD_INIT_VERBS[] = {
			{0x08, AC_VERB_SET_EAPD_BTLENABLE, 0x02},
			{0}
		};
#endif
...
#else
		.v.verbs = STAC_9200_EAPD_INIT_VERBS,
#endif
	},
};
...
#ifdef TARGET_OS2
static const struct hda_pintbl STAC_92HD73XX_ASUS_MOBO_PINS[] = {
...
#ifdef TARGET_OS2
static const struct hda_pintbl STAC_HP_ENVY_BASS_PINS[] = {
			{ 0x0f, 0x90170111 },
			{0}
		};

static const struct hda_pintbl STAC_HP_ENVY_TS_BASS_PINS[] = {
			{ 0x10, 0x92170111 },
			{0}
};
#endif
...
#else
		.v.pins = STAC_HP_ENVY_TS_BASS_PINS,
#endif
	},
	[STAC_HP_ENVY_TS_DAC_BIND] = {
		.type = HDA_FIXUP_FUNC,
...
#ifdef TARGET_OS2
static const struct hda_pintbl STAC_DELL_BIOS_PINS[] = {
...
static const struct hda_pintbl STAC_DELL_BIOS_AMIC_PINS[] = {
			/* configure the analog microphone on some laptops */
			{ 0x0c, 0x90a79130 },
			{0}
		};

static const struct hda_pintbl STAC_DELL_BIOS_SPDIF_PINS[] = {
...
#else
		.v.pins = STAC_DELL_BIOS_AMIC_PINS,
#endif
		.chained = true,
...
#else
		.v.pins = STAC_DELL_BIOS_SPDIF_PINS,
#endif
...
#ifdef TARGET_OS2
static const struct hda_verb STAC_92HD95_HP_BASS_VERBS[] = {
...
#else
		.v.verbs = STAC_92HD95_HP_BASS_VERBS,
#endif
		.chained = true,
