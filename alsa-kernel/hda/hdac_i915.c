...
static bool connectivity_check(struct pci_dev *i915, struct pci_dev *hdac)
{
	...
	bus_b = bus_b->parent;
	if (!bus_a || !bus_b){
dev_info(&hdac->dev, "connectivity_check\n");
		return false;}
	bus_a = bus_a->parent;
	bus_b = bus_b->parent;
	if (bus_a && bus_a == bus_b)
		return true;
dev_info(&hdac->dev, "connectivity_check2\n");
	return false;
}
...
dev_info(&hdac_pci->dev, "i915_gfx_present\n");
if (display_dev->vendor == PCI_VENDOR_ID_INTEL) dev_info(&hdac_pci->dev, "i915_gfx_present2\n");
if ((display_dev->class >> 16) == PCI_BASE_CLASS_DISPLAY) dev_info(&hdac_pci->dev, "i915_gfx_present3\n");
if (connectivity_check(display_dev, hdac_pci)) dev_info(&hdac_pci->dev, "i915_gfx_present4\n");
...
int snd_hdac_i915_init(struct hdac_bus *bus)
{
	...
	if (!acomp){
		dev_info(bus->dev, "ENODEV from !acomp\n");
		return -ENODEV;}
#ifndef TARGET_OS2
	if (!acomp->ops) {
