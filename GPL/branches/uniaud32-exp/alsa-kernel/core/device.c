...
static struct snd_device *look_for_dev(struct snd_card *card, void *device_data)
{
	...
	list_for_each_entry(dev, &card->devices, list, struct snd_device)
...
void snd_device_disconnect_all(struct snd_card *card)
{
	...
	if (snd_BUG_ON(!card))
		return;
	list_for_each_entry_reverse(dev, &card->devices, list, struct snd_device)
		__snd_device_disconnect(dev);
}
...
void snd_device_free_all(struct snd_card *card)
{
	...
	if (snd_BUG_ON(!card))
		return;
	list_for_each_entry_safe_reverse(dev, next, &card->devices, list, struct snd_device) {
