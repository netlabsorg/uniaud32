...
#else
static int position_fix[SNDRV_CARDS] = {-1,-1,-1,-1,-1,-1,-1,-1};
...
static int param_set_xint(const char *val, const struct kernel_param *kp)
{
	...
	mutex_lock(&card_list_lock);
	list_for_each_entry(hda, &card_list, list, struct hda_intel) {
		chip = &hda->chip;
...
#ifdef TARGET_OS2 // modified code from 3.13 otherwise we don't detect hardware. Is schedule_work a stub?
	if (schedule_probe) {
